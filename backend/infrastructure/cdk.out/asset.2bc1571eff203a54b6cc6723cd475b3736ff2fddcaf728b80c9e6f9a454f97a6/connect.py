...
from common.db import query_items, put_item, get_item, get_member_by_id, get_pro_by_id, table
...
def compute_llm_recommendations(goal, pros):
    ...
    if output:
        try:
            # Clean output block if contains markdown or text around JSON array
    ...
    try:
        ...
        # Route: GET /pros/{memberId}/requests
            ...
            response = table.query(
                IndexName="GSI1",
                KeyConditionExpression=Key("GSI1PK").eq(f"PRO#{user_id}") & 
                                       Key("GSI1SK").begins_with("CONNECTION#")
            )
