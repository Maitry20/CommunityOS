...
def calculate_radar_scores(community_id):
    ...
    # Aggregate questions
    for q in questions:
        topic_raw = q.get("topic", "").lower().strip()
        canonical = canonical_topics.get(topic_raw)
        if not canonical and topic_raw:
            # Fallback fuzzy substring match
            for k, v in canonical_topics.items():
                if k in topic_raw or (len(topic_raw) >= 3 and topic_raw in k):
                    canonical = v
                    break
        if not canonical:
    ...
    for p in pros:
        ...
        prev_helps = [h.get("topic", "").lower() for h in p.get("previousHelp", []) if isinstance(h, dict) and h.get("topic")]
        matched_canonicals = set()
        for k, v in canonical_topics.items():
        ...
        # Route: GET /radar/gaps/{gapId}
            ...
            for p in all_pros:
                ...
                for h in prev_helps:
                    ...
                    h_topic = (h.get("topic") or "").lower() if isinstance(h, dict) else ""
                    if h_topic and (h_topic in matched_topic["name"].lower() or matched_topic["name"].lower() in h_topic):
                        recommended_speakers.append({
                            "memberId": p.get("id"),
                            "name": p.get("name"),
                            "role": p.get("title") or p.get("roleTitle") or "Pro",
                            "experienceLine": f"Solved a similar {h.get('topic')} problem {h.get('timeframe', '')}".strip()
                        })
                        break
